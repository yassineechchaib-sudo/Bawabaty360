...
function initIndex() {
  ...
  const urgentJobs = window.concoursData
    .filter(article => {
      if (!article.deadline) return false;
    ...
    .sort((a, b) => {
      const diffA = Math.ceil((new Date(a.deadline) - today) / (1000 * 60 * 60 * 24));
      const diffB = Math.ceil((new Date(b.deadline) - today) / (1000 * 60 * 60 * 24));
      return diffA - diffB;
    })
    .slice(0, 6);

  if (urgentJobs.length > 0) {
    container.innerHTML = urgentJobs.map(renderConcoursCard).join('');
  } else {
    container.innerHTML = '<p style="text-align:center; color:#64748b;">لا توجد عروض مستعجلة حالياً.</p>';
  }
...
function initConcours() {
  ...
  function renderPaginated(container, allArticles) {
    if (!container) return;
    let currentIndex = 0;
    ...
    function showNextBatch() {
      ...
      container.insertAdjacentHTML('beforeend', nextBatch.map(renderConcoursCard).join(''));
  ...
  renderPaginated(document.getElementById('recent-concours-container'), data);

  [6, 8, 9, 10, 11].forEach(echelle => {
...
function initPostesResponsabilite() {
  ...
  // هذه هي الإضافة الوحيدة - تعمل تماماً مثل emplois-superieurs
  document.querySelectorAll('#postes-responsabilite-container .detail-link').forEach(link => {
    link.addEventListener('click', (e) => {
      e.preventDefault();
      const card = link.closest('.concours-card');
      if (card) {
        const articleId = card.dataset.articleId;
        const article = window.postesRespData.find(a => a.id == articleId);
        if (article) showArticleDetail(article);
      }
    });
  });
...
function showPrivateDetail(offer) {
  ...
  const missionsList = offer.mission ? offer.mission.map(m => `<li>${m}</li>`).join('') : '';
  const conseilsList = offer.conseilsCandidature ? offer.conseilsCandidature.map(c => `<li>${c}</li>`).join('') : '';
  
  content.innerHTML = `
    <article class="article-detail">
      <div class="article-header">
        <h2>${offer.fullTitle || offer.title}</h2>
        <div class="card-meta"><i class="fas fa-building"></i> ${offer.company} · <i class="fas fa-map-marker-alt"></i> ${offer.city}</div>
        <div class="card-meta"><i class="fas fa-briefcase"></i> ${offer.contract} · ${offer.level}</div>
        ${offer.salary ? `<div class="card-meta"><i class="fas fa-money-bill"></i> ${offer.salary} MAD</div>` : ''}
      </div>
      ...
      <div class="article-gallery">
        ${offer.images.map(img => `
          <div class="gallery-item">
            <img src="${img}" alt="${offer.title}" onerror="this.style.display='none'">
          </div>
        `).join('')}
      </div>
      ` : ''}
      
      ${offer.introText ? `<p class="intro-text">${offer.introText}</p>` : ''}
      ...
      <ul class="conditions-list">${missionsList}</ul>
      ` : ''}
      ...
      ${conseilsList ? `
      <h3>نصائح الترشيح</h3>
      <ul class="pieces-list">${conseilsList}</ul>
      ` : ''}
      
      ${offer.carriere ? `
      <h3>المسار المهني وآفاق التطور</h3>
      <p>${offer.carriere}</p>
      ` : ''}
      ...
      <a href="${offer.lienInscription || '#'}" target="_blank" class="btn btn-primary btn-large"><i class="fas fa-paper-plane"></i> تقديم الطلب</a>
      
      <div class="share-actions">
        <span>شارك هذا العرض:</span>
        <div class="share-buttons-bottom">
          <button class="share-btn facebook" id="share-facebook"><i class="fab fa-facebook-f"></i> فيسبوك</button>
          <button class="share-btn whatsapp" id="share-whatsapp"><i class="fab fa-whatsapp"></i> واتساب</button>
          <button class="share-btn copy" id="copy-link"><i class="fas fa-link"></i> نسخ الرابط</button>
  ...
  document.getElementById('share-whatsapp')?.addEventListener('click', () => {
    window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(offer.title + ' ' + currentUrl)}`, '_blank');
  });
